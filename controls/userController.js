...
module.exports = {
            ...
            if (userFound) {
              otp.deleteOne({ email: email }).then(() => {
                otp
                  ...
                  .then(() => {
                    res.redirect(
                      `/otpPage?name=${User.name}&email=${User.email}&phone=${User.phone}&password=${User.password}`
                    );
                  });
              });
            } else {
  ...
  forgotNewPassword: async (req, res) => {
    try {
      ...
      if (password === req.body.conPassword) {
        ...
        res.render("user/forgot-new-password", {
          email,
    ...
    try {
      ...
      if (data.newPassword === data.conNewPassword) {
        ...
        if (passwordMatch) {
          ...
          user
            .updateOne({ email: session }, { $set: { password: hashPassword } })
            .then(() => {
              req.session.destroy();
              res.redirect("/");
            });
  ...
  orderedProduct: async (req, res) => {
    ...
    res.render("user/orderd-product", {
      productData,
      countInCart,
      countWishlist,
    });
